...
/* ── Action buttons ──
 * دور السفري ينفّذ ready → delivered فقط (تسليم الطلب للزبون).
 * pending → preparing مسؤولية المطبخ، preparing → ready مسؤولية الميدان.
 */
function OrderActions({ order }: { order: LiveOrder }) {
  ...
  async function handleDeliver() {
    setLoading(true);
    try { await markDelivered(order.id); } finally { setLoading(false); }
  }

  // متاح فقط للطلب الجاهز
  ...
  // pending / preparing: عرض فقط — بانتظار المطبخ ثم الميدان
  const waitLabel = order.status === "pending" ? "بانتظار المطبخ" : "قيد التحضير";
  return (
    <div className="w-full mt-3 h-11 rounded-2xl bg-surface-elevated border border-border
      text-text-muted text-xs font-semibold flex items-center justify-center gap-1.5">
      <PackageCheck className="w-4 h-4" /> {waitLabel}
    </div>
  );
}
